...
public class HttpsqsClient {
	...
	public HttpsqsStatus getStatus(String queueName) throws HttpsqsException {
		...
		if (matcher.find()) {
			...
			status.putLap = Long.parseLong(matcher.group(4));
			status.putPosition = Long.parseLong(matcher.group(5));
			status.getLap = Long.parseLong(matcher.group(6));
			status.getPosition = Long.parseLong(matcher.group(7));
